...
<template>
  <div class="schedule" @dragover="disableDragendAnimation">
    <div>
      <div class="sc-rows" :style="{width: settingData.titleDivW + '%', height: contentH + 'px'}">
      ...
      <div class="sc-main-box" :style="{width: (100 - settingData.titleDivW) + '%'}">
        <div class="sc-main-scroll" :style="{width: contentW + 'px'}">
          <div class="sc-main">
            <div
              class="timeline"
...
const reservedDiv = {
  ...
  created() {
    if (
      new Date(this.startText) - new Date(this.minDate) < 0 &&
  ...
  methods: {
     ...
     */
    setLeftPosition() {
      let leftDiff = this.getMinutesDiff(
     ...
     */
    setWidth() {
      let rightDiff = this.getMinutesDiff(
        new Date(this.startText),
        new Date(this.endText)
      );
...
export default {
  ...
  created() {
    ...
    this.padding =
      this.settingData.dateDivH +
      this.settingData.timeDivH +
      this.settingData.borderW * 4;
    this.dateDivW =
      this.settingData.unitDivW * oneDayCnt +
      (oneDayCnt - this.settingData.borderW);
    this.contentH =
      this.padding +
      (this.settingData.rowH + this.settingData.borderW * 2) *
        this.scheduleData.length;
    this.contentW =
      this.dateDivW * this.dateCnt + this.dateCnt * this.settingData.borderW;
    this.timeDivW =
      (60 / this.settingData.unit) *
        (this.settingData.unitDivW + this.settingData.borderW) -
      1;
  },
  methods: {
    /**
